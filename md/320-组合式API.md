...
教程只讲了 `methods()` 、 `onMounted()` 和 `watch` 的重构，而实际上 `computed()` 等其他部分都是可以改写为组合式 API 的。因此再一次建议阅读 [官方文档](https://v3.cn.vuejs.org/guide/composition-api-introduction.html)，里面有入门所需要的绝大部分内容。
