...
前后端分离项目涉及到一个重要的概念：**序列化**（后面讲解）。好在 Django 世界有一个非常好用的库djangorestframework，简称 drf。它可以帮我们封装好底层序列化的所有实现，让开发者专注于业务本身。
...
接着还需要添加 DRF 的登录视图，以便你后续使用可浏览的接口网页：

> 作用是提供一个登录的入口。后续开发出接口页面后试着把这行代码删掉，看看会有什么不同。
...
前后端分离的核心思想之一，就是两端交互不通过模板语言，而仅仅只传输需要的数据。因此问题就来了。

在 Django 程序的运行过程中，所有的变量都是存储在服务器的内存中；更要命的是，后端 Django 程序是 Python 变量，而前端的浏览器中是 Javascript 变量，这两者是非常不同的（就像在两个不同的时空）。因此需要规定一个“标准格式”，前后端都根据这个标准格式，对资源进行保存、读取、传输等操作。
...
Restful 的处理流程其实也差不多的，不同的是增加了一步，即对数据**序列化**的处理：
