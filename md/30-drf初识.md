...
> 具体含义你可以暂时不管他。后续开发出接口页面后试着把这行代码删掉，看看会有什么不同。
...
在 Django 程序的运行过程中，所有的变量都是存储在服务器的内存中；更要命的是，后端 Django 程序是 Python 变量，而前端的浏览器中是 Javascript 变量，这两者是非常不同的（就像在两个不同的时空）。因此需要规定一个“标准格式”（星际穿越？），前后端都根据这个标准格式，对资源进行保存、读取、传输等操作。
