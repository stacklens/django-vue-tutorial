...
前面章节中我们已经有了文章列表的实现了，接下来就用**文章详情**接口来练练手。
...
代码并不复杂，就是提供了对文章详情的**获取**、**修改**、**删除**的 3 个方法，以及 1 个用于获取具体文章 model 的辅助方法。
...
使用 Mixin 已经足够简单了，但我们还可以用通用视图，让它更简单：
...
> 除了上述介绍的以外，框架提供 `ListModelMixin`、`CreateModelMixin` 等混入类或通用视图，覆盖了基础的增删改查需求。