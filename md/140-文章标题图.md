即使是一个最简单的博客项目，也绕不开文件的上传与下载，比如说博文的标题图片。很遗憾，Json 格式不能够直接处理文件流。

很多开发者用 DRF 框架处理文件上传还是沿用了 Django 的老路子，即用 `multipart/form-data` 发送夹杂着元数据的文件数据。这种方法可行，但在主要接口中发送编码文件总感觉不太 RESTful 。
