...
**文章更新**实际上和**发表文章**非常类似，因此有了前面的基础，就比较简单了。**删除**更是容易，一个 delete 请求就搞定了。
...
- Vue 管理的数据中多了 `articleID` ，用于获取已有文章的索引，以便更新或删除时使用。
...
> 代码相似就说明有优化和合并的可能。教程就不作赘述了，留给读者自行优化的空间。
>
...
文章的增删改查就做完了，感觉如何？如果还觉得迷迷糊糊，那就试着完成分类的增删改查吧，套路都是相同的。如果你可以不看资料独立完成，那么恭喜你，知识掌握得比较扎实了。

> 老实讲分类的增删改查不做也可以，毕竟博客而已，大不了后台改就 ok 了。教程就不讲重复的内容了，读者自行完成吧。