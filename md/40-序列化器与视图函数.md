...
**Windows用户注意**：由于 Windows 系统旧的 `Terminal` 命令行各种与 `httpie` 的兼容问题，后续文章中凡是提到 `httpie` 发送请求所指的命令行，均是指 `PowerShell` 命令行。

教程为了简洁，会采用 httpie 为主、可视化页面为辅的方式进行开发测试。

新手学习更推荐使用 [Postman](https://www.postman.com/downloads/)，可视化界面简单粗暴，省去了大量学习成本。
