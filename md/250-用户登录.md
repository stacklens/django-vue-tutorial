...
`access` 是真正用于用户身份认证的令牌。但此初始令牌有效时间通常比较短（安全考虑），过期后可用 `refresh` 令牌重新获得一个更长有效期的认证令牌。
...
> 课后作业：当前代码在刷新令牌过期后的第一次请求时，会因为进入到 `axios.catch()` 语句导致控制台报错（令牌失效的错误，对实际功能无影响）。请试着优化它，使其不再报错。
