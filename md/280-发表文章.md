...
细节就由读者自己去慢慢啃了，把**新出现的内容和主要逻辑**理出来讲讲：
...
> 还记得吗，我们的后端对标签的处理非常优秀：可以在创建文章的接口里添加标签的列表，并且很好的处理了列表中已有标签和未有标签的混合。如果有则引用已有标签，没有则创建一个。
...
虽然博客的外观不咋样（素雅型），不过核心功能都较完整的实现了，这才是重要的。